...
void *filosofo(void *num) {
	...
	while (1) {
		...
		print_estado();
		//Intentar coger los dos palillos 
		pthread_mutex_lock(&mutex); // Espera para poder modificar el estado
			//Si alguno de los palillos de los laterales del filósofo está ocupado	
		...
		//Imprimir que el filósofo está comiendo
		estado_filosofo[fil_id] = 2;
		printf("Filósofo %d comiendo\n", fil_id);
		print_estado();	
		sleep( (rand() % 2) + 1);
		//Dejar libres los palillos
